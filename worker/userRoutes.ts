...
export function userRoutes(app: Hono<{ Bindings: Env }>) {
     ...
     * POST /api/sync
     * Body: { queue: SyncQueueItem[] }
     */
    app.post('/api/sync', async (c) => {
        try {
            const { queue } = await c.req.json<{ queue: SyncQueueItem[] }>();
            if (!Array.isArray(queue)) {
                return c.json({ success: false, error: 'Invalid queue format' }, { status: 400 });
            }
            const controller = getAppController(c.env);
            const result = await controller.processSyncQueue(queue);
