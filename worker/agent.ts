...
export class ChatAgent extends Agent<Env, ChatState> {
  ...
  async onRequest(request: Request): Promise<Response> {
    try {
      ...
      if (method === 'POST' && url.pathname === '/import') {
        return this.handleBulkImport(await request.json<{ messages: Message[] }>());
      }
  ...
  private async handleChatMessage(body: { message: string; model?: string; stream?: boolean }): Promise<Response> {
    ...
    const userMessage = createMessage('user', message.trim(), this.state.sessionId);
    this.setState({
      ...this.state,
      ...
      if (stream) {
        ...
        (async () => {
          try {
            ...
            const assistantMessage = createMessage('assistant', response.content, this.state.sessionId, response.toolCalls);
            ...
            try {
              ...
              const errorMsg = createMessage('assistant', errorMessage, this.state.sessionId);
              this.setState({
                ...this.state,
  ...
  /**
   * Handle bulk import of messages for sync
   */
  private async handleBulkImport(body: { messages: Message[] }): Promise<Response> {
    const { messages } = body;
    if (!Array.isArray(messages)) {
      return Response.json({ success: false, error: 'Invalid messages format' }, { status: 400 });
    }
    // A simple append strategy. For more complex sync, you might merge based on timestamps.
    this.setState({ ...this.state, messages: [...this.state.messages, ...messages] });
